...
function updateVariantInCart(cartLineItem: ShopifyBuy.LineItem, quantity: number) {
    var variantId: string | number = cartLineItem.variant.id;
    ...
    client.checkout
        .updateLineItems(cartLineItem.id, [lineItemVariant])
        .then(function (updatedCart: ShopifyBuy.Cart) {
            ...
            if (updatedCart.lineItems.length >= cartLength) {
                ...
                $cartItem.find('.cart-item__price').text(formatAsMoney(cartLineItem.variant.price));
            } else {
                $cartItem
                    .addClass('js-hidden')
...
function renderCartItem(lineItem: ShopifyBuy.LineItem) {
    ...
    var itemImage = lineItem.variant.image.src;
    $lineItemTemplate.attr('data-variant-id', lineItem.variant.id);
    $lineItemTemplate.addClass('js-hidden');
    $lineItemTemplate.find('.cart-item__img').css('background-image', 'url(' + itemImage + ')');
    $lineItemTemplate.find('.cart-item__title').text(lineItem.title);
    $lineItemTemplate.find('.cart-item__variant-title').text(lineItem.variant.title);
    $lineItemTemplate.find('.cart-item__price').text(formatAsMoney(lineItem.variant.price));
...
function updateCartTabButton() {
    if (cart.lineItems.length > 0) {
        $('.btn--cart-tab .btn__counter').html('' + cart.lineItems.length);
